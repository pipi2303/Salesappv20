// GET/PUT/DELETE /api/distributors/:id — single-record operations,
// including the Bab 9 approve/reject decision (a status change away
// from PENDING auto-stamps decidedById/decidedAt; nothing else on this
// route does).
...
export default async function handler(req: ApiRequest, res: ApiResponse) {
  const id = getId(req);
  if (!id) {
    res.status(400).json({ success: false, error: 'Missing distributor id' });
    return;
  }
  ...
  try {
    ...
    console.error('[api/distributors/[id]] unexpected error:', err);
